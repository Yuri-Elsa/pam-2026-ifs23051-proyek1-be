...
import org.delcom.watchlist.data.AuthTokenRequest
...
class AuthService(
    ...
    suspend fun postLogin(call: ApplicationCall) {
        ...
        authTokenRepository.deleteByUserId(existUser.id)
    ...
    suspend fun postRefreshToken(call: ApplicationCall) {
        ...
        val existing = authTokenRepository.getByAuthAndRefreshToken(
            request.authToken, request.refreshToken
        ) ?: throw AppException(401, "Token tidak valid!")

        // Hapus token lama
        authTokenRepository.deleteByAuthToken(request.authToken)
